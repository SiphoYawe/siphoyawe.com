...
/**
 * macOS-dock social row (danielsun forensics): the hovered icon lifts and
 * scales out of the row with a label revealed beneath; neighbours stay put.
 * The controlled third loud moment, kept tasteful.
 */
export function SocialDock({ className = "" }: { className?: string }) {
  const reduce = useReducedMotion();

  return (
    <ul className={`flex items-end gap-3 ${className}`} aria-label="Social links">
      {SOCIALS.map(({ label, href, icon }) => (
        <li key={label} className="relative">
          ...
          >
            ...
            <span
              aria-hidden
              className="pointer-events-none absolute top-full mt-2 rounded-md bg-sable px-2 py-0.5 font-display text-[11px] font-semibold text-paper opacity-0 transition-all duration-150 group-hover:translate-y-0.5 group-hover:opacity-100 group-focus-visible:translate-y-0.5 group-focus-visible:opacity-100 dark:bg-paper dark:text-sable"
            >
              {label}
            </span>
          </motion.a>
